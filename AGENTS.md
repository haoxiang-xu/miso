# Unchain Harness - Codex Project Instructions
...
- **`unchain`** — Legacy source tree: Broth engine, characters, workspace pins (retained for backward compat)
...
2. **Unchain Broth** (`unchain/runtime/engine.py`): Legacy engine, 157KB monolith

PuPu (the Electron frontend) uses unchain agents via `unchain_runtime/server/unchain_adapter.py`.
...
### File Layout
- Source: `src/unchain/` and `src/unchain/`
- Tests: `tests/`
- Docs: `docs/en/` and `docs/zh-CN/`
- Model configs: `src/unchain/runtime/resources/model_capabilities.json` and `model_default_payloads.json`
- Toolkit manifests: `src/*/toolkits/builtin/*/toolkit.toml`

### Adding a New Model
1. Add entry to `src/unchain/runtime/resources/model_capabilities.json`
2. Add default payload to `src/unchain/runtime/resources/model_default_payloads.json`
3. If model has a different API name, set `provider_model` field
4. Update `src/unchain/schemas/models.py` if adding a named constant
...
| `src/unchain/runtime/engine.py` | Broth — legacy engine (157KB) |
| `src/unchain/runtime/resources/model_capabilities.json` | Model registry |
| `src/unchain/characters/character.py` | CharacterAgent system |

...
def _normalize_string_list(
    ...
    *,
    ...
    if not raw_items and fallback:
        raw_items = [item.strip() for item in fallback if isinstance(item, str) and item.strip()]
...
def _status_defaults(status: str) -> dict[str, Any]:
    return _STATUS_DEFAULTS.get(status, _STATUS_DEFAULTS["free"])


def _serialize_dataclass(value: object) -> object:
    ...
    if isinstance(value, dict):
        return {key: _serialize_dataclass(inner) for key, inner in value.items()}
    return copy.deepcopy(value)
...
class CharacterScheduleBlock:
    ...
    def coerce(cls, value: object) -> CharacterScheduleBlock:
        ...
        defaults = _status_defaults(status)
        ...
        return cls(
            ...
            availability=_coerce_availability(
                ...
                fallback=defaults["availability"],
            ),
            interruption_tolerance=_clamp_unit_interval(
                raw.get("interruption_tolerance"),
                defaults["interruption_tolerance"],
            ),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "courtesy_message": self.courtesy_message,
...
class CharacterSchedule:
    ...
    def coerce(
        ...
        *,
        ...
        defaults = _status_defaults(default_status)
        ...
        return cls(
            ...
            default_availability=_coerce_availability(
                raw.get("default_availability"),
                fallback=defaults["availability"],
            ),
            default_interruption_tolerance=_clamp_unit_interval(
                raw.get("default_interruption_tolerance"),
                defaults["interruption_tolerance"],
            ),
            default_reply_mode=_coerce_reply_mode(
                raw.get("default_reply_mode"),
                fallback="auto",
            ),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "blocks": [_serialize_dataclass(block) for block in self.blocks],
...
class CharacterSpec:
    ...
    @classmethod
    def coerce(cls, value: object) -> CharacterSpec:
        if isinstance(value, cls):
            return copy.deepcopy(value)
        raw = value if isinstance(value, dict) else {}
        ...
        character_id = sanitize_character_key_component(
            raw.get("id"),
            fallback=sanitize_character_key_component(name, fallback="character"),
        )
        schedule = CharacterSchedule.coerce(
            raw.get("schedule"),
        ...
        return cls(
            ...
            speaking_style=_normalize_string_list(raw.get("speaking_style")),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "speaking_style": list(self.speaking_style),
...
def _block_is_active(block: CharacterScheduleBlock, now: datetime) -> bool:
    ...
    if start_minutes <= end_minutes:
        return (
            _day_matches(current_day, block.days)
...
def _active_schedule_block(
    schedule: CharacterSchedule,
    now: datetime,
) -> CharacterScheduleBlock | None:
...
def _select_active_obligation(
    obligations: Sequence[CharacterObligation],
    *,
    now: datetime,
    timezone_name: str,
) -> CharacterObligation | None:
    ...
    return sorted(
        ...
        key=lambda item: (
            item.priority,
            1.0 - item.interruption_tolerance,
            item.status,
        ),
        reverse=True,
    )[0]
...
def _base_schedule_state(
    schedule: CharacterSchedule,
    now: datetime,
) -> tuple[str, str, float, str, str, CharacterScheduleBlock | None]:
    active_block = _active_schedule_block(schedule, now)
    if active_block is None:
        return (
            schedule.default_status,
            schedule.default_availability,
            schedule.default_interruption_tolerance,
            schedule.default_reply_mode,
            schedule.default_courtesy_message,
            None,
        )
    return (
        active_block.status,
        active_block.availability,
        active_block.interruption_tolerance
        if active_block.interruption_tolerance is not None
        else schedule.default_interruption_tolerance,
        active_block.reply_mode or schedule.default_reply_mode,
        active_block.courtesy_message or schedule.default_courtesy_message,
        active_block,
    )


def _compose_courtesy_message(
    ...
    *,
    ...
    if availability == "offline":
        return f"{character.name} is unavailable right now."
    if available_at:
        local_dt = _parse_datetime(available_at, character.timezone)
        when_text = local_dt.strftime("%a %H:%M") if local_dt is not None else available_at
        return f"{character.name} is busy with {status} right now and may respond after {when_text}."
    return f"{character.name} is busy with {status} right now."
...
def _next_available_at(
    ...
    *,
    ...
    for _ in range(1, 7 * 24 * 4 + 1):
        ...
        if evaluation.availability in {"available", "limited"}:
            return evaluation.at
    return None


def evaluate_character(
    character: CharacterSpec | dict[str, Any],
    *,
    now: datetime | str | None = None,
    obligations: Sequence[CharacterObligation | dict[str, Any]] | None = None,
    include_next_available: bool = True,
) -> CharacterEvaluation:
    spec = CharacterSpec.coerce(character)
    local_now = _coerce_now(now, spec.timezone)
    normalized_obligations = tuple(
        item
        if isinstance(item, CharacterObligation)
        else CharacterObligation.coerce(item, timezone_name=spec.timezone)
    ...
    (
        ...
        courtesy_message,
        active_block,
    ) = _base_schedule_state(spec.schedule, local_now)

    reasons = [f"schedule:{status}"]
    active_obligation = _select_active_obligation(
    ...
    return CharacterEvaluation(
        ...
        courtesy_message=courtesy_message or None,
...
def decide_character_response(
    ...
    *,
    ...
    spec = CharacterSpec.coerce(character)
    current_evaluation = evaluation or evaluate_character(
        spec,
        now=now,
        obligations=obligations,
    )
    ...
    social_drive = (
        (spec.talkativeness * 0.6)
        + (spec.politeness * 0.25)
        + ((1.0 - spec.autonomy) * 0.15)
    )

    if explicit_mode != "auto":
        action = explicit_mode
        ...
        if current_evaluation.interruption_tolerance >= 0.75 and social_drive >= 0.5:
            action = "reply"
            reason = "interruptible_busy_state"
        elif spec.politeness >= 0.5:
            action = "defer"
            reason = "busy_but_polite"
        else:
            action = "ignore"
            reason = "busy_and_unavailable"
    elif current_evaluation.availability == "limited":
        if (
            social_drive >= 0.45
            or current_evaluation.interruption_tolerance >= 0.55
            or spec.politeness >= 0.6
        ):
            action = "reply"
            reason = "limited_but_responsive"
        else:
            action = "defer"
            reason = "limited_attention"
    else:
        if social_drive >= 0.3 or spec.politeness >= 0.4:
            action = "reply"
            reason = "available_and_willing"
        elif spec.autonomy >= 0.85 and spec.talkativeness <= 0.2:
            action = "ignore"
            reason = "autonomous_and_reserved"
        else:
            action = "defer"
            reason = "available_but_disinclined"

    courtesy_message = None
    if action == "defer":
        courtesy_message = _compose_courtesy_message(
            spec,
            availability=current_evaluation.availability,
            status=current_evaluation.status,
            available_at=current_evaluation.available_at,
            fallback_message=current_evaluation.courtesy_message or "",
        )
...
def _build_character_instructions(
    ...
    *,
    ...
    availability_suffix = (
        f" Available after {evaluation.available_at}."
...
class CharacterAgent:
    ...
    def build(
        *,
    ...
    ) -> CharacterAgentBundle:
        spec = CharacterSpec.coerce(character)
        safe_session_id = _trimmed(session_id, fallback="default")
        ...
        instructions = _build_character_instructions(
            spec,
        ...
        agent = Agent(
            ...
            defaults=copy.deepcopy(defaults) if isinstance(defaults, dict) else None,
            broth_options=copy.deepcopy(broth_options) if isinstance(broth_options, dict) else None,
            toolkit_catalog_config=toolkit_catalog_config,
        )
        return CharacterAgentBundle(
            agent=agent,
            instructions=instructions,
    ...
    def build_config(
        *,
        ...
        return build_character_agent_config(
            character=spec,
            session_id=bundle.session_id,
            self_namespace=bundle.self_namespace,
            relationship_namespace=bundle.relationship_namespace,
            evaluation=bundle.evaluation,
            decision=bundle.decision,
            instructions=bundle.instructions,
            self_profile=bundle.self_profile,
            relationship_profile=bundle.relationship_profile,
        )

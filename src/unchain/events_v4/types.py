...
VisibilityV4 = Literal["user", "debug", "internal"]
...
RUNTIME_EVENT_TYPES_V4: frozenset[str] = frozenset(
    {
        "session.started",
        "run.started",
        "run.completed",
...
VISIBILITIES_V4: frozenset[str] = frozenset({"user", "debug", "internal"})
...
class RuntimeEventLinksV4:
    ...
    @classmethod
    def from_dict(cls, raw: Any) -> "RuntimeEventLinksV4":
...
class RuntimeEventV4:
    ...
    def __post_init__(self) -> None:
        ...
        if self.type not in RUNTIME_EVENT_TYPES_V4:
            raise ValueError(f"unknown event type: {self.type}")
        if self.visibility not in VISIBILITIES_V4:
            raise ValueError(f"visibility must be one of {sorted(VISIBILITIES_V4)}")
    ...
    @classmethod
    def from_dict(cls, raw: dict[str, Any], *, strict: bool = True) -> "RuntimeEventV4":
        if not isinstance(raw, dict):
        ...
        if strict and event_type not in RUNTIME_EVENT_TYPES_V4:
            raise ValueError(f"unknown event type: {event_type}")
        visibility = raw.get("visibility", "user")
        if visibility not in VISIBILITIES_V4:
            raise ValueError(f"visibility must be one of {sorted(VISIBILITIES_V4)}")
        ...
        return cls(
            ...
            links=RuntimeEventLinksV4.from_dict(raw.get("links")),
